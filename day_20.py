...
# Python语言最可爱的地方在于它的标准库和三方库实在是太丰富了，日常开发工作中的很多任务都可以通过这些标准库或者三方库直接解决。
# 下面我们先介绍Python标准库中的一些常用模块，后面的课程中再陆陆续续为大家介绍Python常用三方库的用途和用法。

## base64 - Base64编解码模块
# Base64是一种基于64个可打印字符来表示二进制数据的方法。由于$log _{2}64=6$，所以Base64以6个比特（二进制位，可以表示0或1）为一个单元，每个单元对应一个可打印字符。
# 对于3字节（24比特）的二进制数据，我们可以将其处理成对应于4个Base64单元，即3个字节可由4个可打印字符来表示。
...
#
# 关于Base64编码的细节，大家可以参考[《Base64笔记》](http://www.ruanyifeng.com/blog/2008/06/base64.html)一文，
# Python标准库中的`base64`模块提供了`b64encode`和`b64decode`两个函数，专门用于实现Base64的编码和解码，
# 下面演示了在Python的交互式环境中执行这两个函数的效果。
import base64
...
# - `deque`：双端队列，是列表的替代实现。Python中的列表底层是基于数组来实现的，而`deque`底层是双向链表，因此当你需要在**头尾**添加和删除元素是，`deque`会表现出更好的性能，渐近时间复杂度为O(1)。
...
Card = namedtuple('Card', ('suite', 'face')) ## namedtuple({类名},({类的属性}))来创建类
card1 = Card('红桃', 5)
card2 = Card('草花', 9)
card1
...
## `Counter`类统计列表中出现次数最多的三个元素的例子。from collections import Counter
import collections
words = [
...
counter = collections.Counter(words) ## 生成一个dict，key为word，value为频数
# 打印words列表中出现频率最高的3个元素及其出现次数
for elem, count in counter.most_common(3):
    print(elem, count) ## eyes 8    the 5   look 4

## hashlib - 哈希函数模块
# 哈希函数又称哈希算法或散列函数，是一种为已有的数据创建“数字指纹”（哈希摘要）的方法。
# 哈希函数把数据压缩成摘要，对于相同的输入，哈希函数可以生成相同的摘要（数字指纹），需要注意的是这个过程并不可逆（不能通过摘要计算出输入的内容）。
# 一个优质的哈希函数能够为不同的输入生成不同的摘要，出现哈希冲突（不同的输入产生相同的摘要）的概率极低，
# [MD5](https://zh.wikipedia.org/wiki/MD5)、[SHA家族]([https://zh.wikipedia.org/wiki/SHA%E5%AE%B6%E6%97%8F](https://zh.wikipedia.org/wiki/SHA家族))就是这类好的哈希函数。

# Python标准库的`hashlib`模块提供了对哈希函数的封装，通过使用`md5`、`sha1`、`sha256`等类，我们可以轻松的生成“数字指纹”。
# 举一个简单的例子，用户注册时我们希望在数据库中保存用户的密码，很显然我们不能将用户密码直接保存在数据库中，这样可能会导致用户隐私的泄露.
# 所以在数据库中保存用户密码时，通常都会将密码的“指纹”保存起来，用户登录时通过哈希函数计算密码的“指纹”再进行匹配来判断用户登录是否成功。

import hashlib
## 计算一组密码的MD5摘要
key_word="123456789"
print(hashlib.md5(key_word.encode()).hexdigest())
...
# note:很多网站在下载链接的旁边都提供了哈希摘要，完成文件下载后，我们可以计算该文件的哈希摘要并检查它与网站上提供的哈希摘要是否一致（指纹比对）。
# 如果计算出的哈希摘要与网站提供的并不一致，很有可能是下载出错或该文件在传输过程中已经被篡改，这时候就不应该直接使用这个文件。
...
## 找出股票价格最高的三只股票
print(heapq.nlargest(3,list2,key=lambda x:x['price'])) ## day_18中对于lambda的描述，lambda x（传入的参数）:参数进行的表达式，表达式计算的结果就是返回值
## 找出持有数量最高的三只股票
print(heapq.nlargest(3,list2,lambda x:x["shares"]))
...
#
# 由于`uuid4`存在概率型重复，那么在真正需要全局唯一标识符的地方最好不用使用它。
# 在分布式环境下，`uuid1`是很好的选择，因为它能够保证生成ID的全局唯一性。下面是在**Python交互式环境中**使用`uuid1`函数生成全局唯一标识符的例子。

import uuid
uuid.uuid1().hex ## '622a8334baab11eaaa9c60f81da8d840'
uuid.uuid1().hex ## '62b066debaab11eaaa9c60f81da8d840'
uuid.uuid1().hex ## '642c0db0baab11eaaa9c60f81da8d840'

# 总结：这里介绍了几个标准库，base64（用于二进制编码解码）、collections（有多种好用的容器数据类型）、hashlib（哈希函数，封装了MD5，SHA1等散列方法）
# heapq（堆排序，快速输出K个最大值和最小值）、itertools（生成迭代器，比如生成排列组合、笛卡尔积、无限循环序列）、random（随机数和随机抽样）
# os.path（路径操作）、uuid（UUID生成）
