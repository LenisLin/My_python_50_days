## day_26
## 用python读写pdf文件
# PDF是Portable Document Format的缩写，这类文件通常使用`.pdf`作为其扩展名。
...
# 当然，`PyPDF2`并不是什么样的PDF文档都能提取出文字来，这个问题就我所知并没有什么特别好的解决方法，尤其是在提取中文的时候。
# 网上也有很多讲解从PDF中提取文字的文章，推荐阅读[《三大神器助力Python提取pdf文档信息》](https://cloud.tencent.com/developer/article/1395339)一文进行了解。
...
## 旋转和叠加页面
# 上面的代码中通过创建`PdfFileReader`对象的方式来读取PDF文档，该对象的`getPage`方法可以获得PDF文档的指定页并得到一个`Page`对象，
# 利用`Page`对象的`rotateClockwise`和`rotateCounterClockwise`方法可以实现页面的顺时针和逆时针方向旋转，代码如下所示。
import PyPDF2
...
## `Page`对象还有一个名为`mergePage`的方法，可以将一个页面和另一个页面进行叠加，
# 对于叠加后的页面，我们还是使用`PdfFileWriter`对象的`addPage`将其添加到一个新的`PDF`文档中，有兴趣的读者可以自行尝试。
...
with open('test.pdf', 'rb') as file:
    ...
    with open('test-encrypted.pdf', 'wb') as file2:
        writer.write(file2)
## note：按照上面的方法也可以读取多个PDF文件的多个页面并将其合并到一个PDF文件中。

## 创建pdf文件
# 创建PDF文档需要三方库`reportlab`的支持，该库的使用方法可以参考它的[官方文档](https://www.reportlab.com/docs/reportlab-userguide.pdf)
...
# 说明：上面的代码使用了很多字面常量来指定位置和尺寸，在商业项目开发中应该避免这样的硬代码（hard code），因为这样的代码不仅可读性差，维护起来也是一场噩梦。
# 如果项目中需要动态生成PDF文档且PDF文档的格式是相对固定的，可以将上面的字面常量处理成符号常量。
# 记住：符号常量优于字面常量。Python语言没有内置定义常量的语法，但是可以约定变量名使用全大写字母的变量就是常量。
## 字面常量就是整形、浮点型和字符串型等常量。符号常量就是#define或者const定义的常量（C语言）
## 意思就是，要让编程语言更加贴近自然语言，可读性更强
